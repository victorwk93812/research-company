...
\textbf{Protocol II (Middle-Out Cat-Uncompute, unitary).} A
...
achieves Bell fidelity $0.928$, MOCU achieves $0.885$, and the
...
\item \textbf{Protocol I — entanglement-swap with feed-forward}
  ...
  primitives.
\item \textbf{Protocol II — Middle-Out Cat-Uncompute (MOCU)}
  (\S\ref{sec:mocu}). A measurement-free, fully unitary alternative that
...
bottom-leg qubits.
(c) A measurement-free companion (Protocol~II / MOCU) saturating the
...
\section{Protocol II: Middle-Out Cat-Uncompute (measurement-free)}
\label{sec:mocu}
...
light-cone). Our Middle-Out Cat-Uncompute (MOCU) saturates this bound
...
\label{par:mocu-parallel}
The reference implementation in \texttt{analysis/mocu.py} executes
...
correctness proof of \S\ref{sec:mocu} carries over verbatim — but it
...
Fig.~\ref{fig:circuit-mocu} shows the explicit MOCU circuit for $L=5$
...
\begin{figure}[h]
\centering
\includegraphics[width=0.78\textwidth]{../analysis/figures/circuit_mocu_L5.pdf}
\caption{Protocol~II (MOCU) on the $L=5$ ladder. Wires
...
\begin{tabular}{l|c c c c c c}
  ...
  Protocol II (MOCU)          & 1     & $2L-1$ & 0 & 0 & $L+2^{\dagger}$ & $L+2^{\dagger}$\\
  \midrule
...
\caption{Closed-form resource scaling of the five protocols on the
  ...
  $^{\dagger}$The MOCU figure is the sequential-schedule depth implemented
  in \texttt{analysis/mocu.py}; with aggressive interleaving of Phase~B
...
\texttt{mocu} (Prot.~II)       & 12 & 19 & 0 & 0.885 \\
...
\caption{Headline numbers at $L=10$. The \texttt{entanglement\_swap}
  ...
  collapse to a single conditional layer (depth $\le 7$). MOCU's
...
out / measured, attenuating their accumulated noise); MOCU sits between
...
\item \textbf{Analytical.} The stabiliser-formalism derivations of
  ...
  (Protocol~I, $N=3$), and \S\ref{sec:mocu} (Protocol~II) establish
...
\paragraph{Protocol II (MOCU).} Preferred when:
\begin{enumerate}
...
\item \texttt{analysis/mocu.py} — Protocol~II unitary middle-out
...
\item \texttt{analysis/draw\_circuits.py} — generates
  Figs.~\ref{fig:circuit-swap} and~\ref{fig:circuit-mocu}.
...
specifications in \S\ref{sec:swap} and \S\ref{sec:mocu}; (iii) running
...
topology of the challenge). \textbf{Protocol~II (MOCU)} is a
